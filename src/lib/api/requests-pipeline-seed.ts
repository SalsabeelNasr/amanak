...
import { REQUEST_TASK_TEMPLATE_TITLES } from "@/lib/services/lead-task-rules";
import type { Patient, Request, RequestDocument, RequestTask, RequestTaskTemplateKey, Quotation } from "@/types";

function nowMinusDays(days: number): string {
  return new Date(Date.now() - days * 24 * 60 * 60 * 1000).toISOString();
}

const STANDARD_DOCS: RequestDocument[] = [
  {
    id: "doc_med",
    type: "medical_report",
    name: "التقرير الطبي",
...
function completedMilestone(
  ...
  return {
    id,
    title: REQUEST_TASK_TEMPLATE_TITLES[key],
    completed: true,
    kind: key,
    source: "system",
    templateKey: key,
...
function makeRequest(
  partial: Partial<Request> &
    Pick<Request, "id" | "patientId" | "treatmentSlug" | "status">,
): Request {
  return {
    recordType: "request",
    statusHistory: [],
...
/** One mock request per {@link RequestStatus} for filter QA; `lead_1` is the patient portal demo. */
export const PIPELINE_MOCK_SEED: Request[] = [
  makeRequest({
    id: "lead_1",
    patientId: "patient_1",
...
/** Deduped patient directory for mock CRM + portal (includes CRM-only prospects without requests). */
export const PATIENT_SEED: Patient[] = [
  ...
  },
  {
    ...
    hasPortalAccess: false,
    createdBy: "crm",
    notes: "Created in CRM — no portal login yet.",
    createdAt: nowMinusDays(3),
    updatedAt: nowMinusDays(3),
  },
];
