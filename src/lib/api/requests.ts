...
import { getPatientByIdSync } from "./patients";
import { PIPELINE_MOCK_SEED } from "./requests-pipeline-seed";
...
export {
  ensureSystemTasks,
  listRequestTaskTemplateKeys,
  processRequestTasks,
  resolveRequestTasksAfterRequestChange,
} from "@/lib/services/lead-task-rules";
...
export type AddRequestTaskAttachmentInput = {
  slotId: string;
  fileName: string;
  sizeBytes: number;
  mockUrl?: string;
...
export type AddRequestTaskInput = {
  title: string;
  dueAt?: string;
  assigneeId?: string;
  ...
  attachments?: AddRequestTaskAttachmentInput[];
};

export type UpdateRequestTaskPatch = Partial<
  Pick<RequestTask, "title" | "completed" | "dueAt" | "assigneeId" | "completionOutcome">
> & {
  /** Merged with existing task `creationFields`. */
  creationFields?: Record<string, string>;
  /** Appended as new {@link RequestTaskAttachment} rows (with generated ids). */
  attachments?: AddRequestTaskAttachmentInput[];
};

export type UpdateRequestTaskOptions = {
 ...
 */
export function sortRequestTasksForDisplay(
  tasks: readonly RequestTask[],
  nowMs: number = Date.now(),
): RequestTask[] {
  const incomplete: RequestTask[] = [];
  const complete: RequestTask[] = [];
  for (const task of tasks) {
    if (task.completed) complete.push(task);
    else incomplete.push(task);
  }

  const activeRank = (task: RequestTask): { group: number; dueKey: number } => {
...
const SEED: Request[] = PIPELINE_MOCK_SEED;

const STORE = new Map<string, Request>();

function persistProcessedRequest(
  prev: Request,
  next: Request,
  at: string,
  opts?: { followUpActor?: MockUser },
): Request {
  const processed = processRequestTasks(prev, { ...next, updatedAt: at }, at);
...
for (const l of SEED) {
  ...
  persistProcessedRequest(l, { ...l, updatedAt: at }, at);
}
...
export async function listRequests(
  ...
  if (filters?.country) {
    ...
    rows = rows.filter((r) => {
      const p = getPatientByIdSync(r.patientId);
      return p?.country.toLowerCase().includes(c) ?? false;
    });
  }
  if (filters?.patientId) {
    rows = rows.filter((r) => r.patientId === filters.patientId);
  }
  return rows.sort(
    (a, b) => new Date(b.updatedAt).getTime() - new Date(a.updatedAt).getTime(),
  );
...
function newRequestTaskId(): string {
  return `task_${globalThis.crypto.randomUUID()}`;
}

function newRequestAppointmentId(): string {
  return `appt_${globalThis.crypto.randomUUID()}`;
}

export type AddRequestAppointmentTreatmentInput = {
  kind: "treatment";
  startsAt: string;
...
export type AddRequestAppointmentOnlineInput = {
  kind: "online_meeting";
  startsAt: string;
  meetingUrl: string;
  title?: string;
...
export type AddRequestAppointmentConsultationInput = {
...
export async function addRequestAppointment(
  ...
  const appointments: RequestAppointment[] = [...existing.appointments];

  let taskToAdd: RequestTask | undefined;
  let appointment: RequestAppointment;
  ...
  return persistProcessedRequest(existing, updated, ts);
}

export const REQUEST_DOCUMENT_TYPE_ORDER: RequestDocument["type"][] = [
  "medical_report",
  "xray",
  "lab_result",
  "previous_operations",
...
function newRequestDocumentId(): string {
  return `doc_${globalThis.crypto.randomUUID()}`;
}

export type UploadRequestDocumentInput = {
...
function mapAttachmentInputsToStored(
  inputs: AddRequestTaskAttachmentInput[],
  ts: string,
): RequestTaskAttachment[] {
  return inputs.map((a) => ({
    id: newAttachmentId(),
...
function leadAttachmentsToCompletionInputs(
  attachments: RequestTaskAttachment[] | undefined,
): {
  slotId: string;
  fileName: string;
  sizeBytes: number;
...
function buildMergedTaskFromPatch(
  ...
  let attachments: RequestTaskAttachment[] | undefined;
...
export async function addRequestTask(
  requestId: string,
  input: AddRequestTaskInput,
  options?: { simulateDelay?: boolean },
): Promise<Request> {
  await applyMockDelay(options?.simulateDelay);
  const existing = STORE.get(requestId);
  ...
  let creationTypeId: RequestTaskCreationTypeId | undefined;
  let creationFields: Record<string, string> | undefined;
  let attachmentInputs: AddRequestTaskAttachmentInput[] = input.attachments ?? [];
  ...
  const attachments: RequestTaskAttachment[] | undefined =
...
export async function updateRequestTask(
  ...
  taskId: string,
  patch: UpdateRequestTaskPatch,
  options?: UpdateRequestTaskOptions,
): Promise<Request> {
  await applyMockDelay(options?.simulateDelay);
  const existing = STORE.get(requestId);
  if (!existing) {
  ...
  if (idx === -1) {
    throw new Error(`Task ${taskId} not found on lead ${requestId}`);
  }
  ...
  const patchBody: UpdateRequestTaskPatch = { ...patch };
  ...
  if (completing) {
    ...
    if (action !== null) {
      ...
      const completedTask: RequestTask = {
        ...merged,
        completed: true,
    ...
    const updated: Request = { ...existing, documents, tasks, updatedAt: ts };
    return persistProcessedRequest(existing, updated, ts);
  }
  ...
  if (reopening) {
    ...
    const tasks = [...existing.tasks];
    tasks[idx] = next;
    const updated: Request = { ...existing, tasks, documents, updatedAt: ts };
    return persistProcessedRequest(existing, updated, ts);
  }
...
export async function deleteRequestTask(
  ...
  const updated: Request = { ...existing, tasks, updatedAt: ts };
  return persistProcessedRequest(existing, updated, ts);
}
...
/**
 * Appends a new {@link Quotation} with `status: "draft"` and sets it as {@link Request.activeQuotationId}.
 */
export async function createDraftQuotation(
  ...
  return persistProcessedRequest(existing, updated, at);
}
...
export async function sendDraftQuotationToPatient(
  ...
  if (idx < 0) {
    throw new Error(`Quotation ${quotationId} not found on lead ${requestId}`);
  }
  ...
  return persistProcessedRequest(existing, next, next.updatedAt);
}
 ...
 * tasks, prefer {@link addRequestTask}, {@link updateRequestTask}, {@link deleteRequestTask}.
 */
...
export type SetRequestStatusOptions = {
  actor: MockUser;
  note: string;
  simulateDelay?: boolean;
};
...
export async function setRequestStatus(
  ...
  options: SetRequestStatusOptions,
): Promise<Request> {
  await applyMockDelay(options.simulateDelay);
  const existing = STORE.get(id);
  if (!existing) {
...
/**
 * Creates a new treatment {@link Request} for an existing {@link Patient} (CRM “+ New request”).
 */
export async function createRequestForPatient(
  patientId: string,
  ...
  await applyMockDelay(options?.simulateDelay);
  if (!getPatientByIdSync(patientId)) {
    throw new Error(`Patient ${patientId} not found`);
  }
  const ts = new Date().toISOString();
  const id = `lead_${globalThis.crypto.randomUUID().replace(/-/g, "").slice(0, 10)}`;
  const { treatmentSlug, ...rest } = input;
  const row: Request = {
    id,
    patientId,
    treatmentSlug,
  ...
  STORE.set(id, row);
  return persistProcessedRequest(row, row, ts);
}
