...
import type { Lead, PackageTier, ProvisionalRequest } from "@/types";
...
export function LeadRequestDetailsCard({ lead }: { lead: Lead }) {
  ...
  const hasDetails = Boolean(req && provisionalRequestHasVisibleDetails(req));
  ...
  const treatmentDisplay = lead.treatmentSlug?.trim()
