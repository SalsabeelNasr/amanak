...
import type { Lead, LeadStatus, LeadTaskTemplateKey } from "@/types";
...
export function LeadsTable({ leads, settings }: { leads: Lead[]; settings: CrmSettings }) {
  ...
  const prioritizedLeads = useMemo(() => {
    ...
    const getLeadScore = (lead: Lead) => {
      ...
      enabledRules.forEach((rule, index) => {
        ...
        switch (rule.id) {
          case "overdue_tasks":
            if (lead.tasks.some(tk => !tk.completed && tk.dueAt && new Date(tk.dueAt).getTime() < startOfToday)) {
              score += weight;
            }
            break;
          case "tasks_due_today":
            if (lead.tasks.some(tk => !tk.completed && tk.dueAt && new Date(tk.dueAt).getTime() >= startOfToday && new Date(tk.dueAt).getTime() < startOfToday + 86400000)) {
              score += weight;
            }
            break;
          case "hot_priority":
          ...
          case "open_matching_tasks": {
            const keys = (rule.taskTemplateKeys ?? []).filter(Boolean) as LeadTaskTemplateKey[];
            if (keys.length === 0) break;
            const openByTemplate = new Set(
              lead.tasks
                .filter((tk) => !tk.completed && tk.templateKey && keys.includes(tk.templateKey))
                .map((tk) => tk.templateKey as LeadTaskTemplateKey),
            );
    ...
    const scoredLeads = leads.map(l => ({ lead: l, score: getLeadScore(l) }));
    ...
    if (sortBy === "prioritize") {
      ...
      return [...leads].sort((a, b) => new Date(a.updatedAt).getTime() - new Date(b.updatedAt).getTime());
    }
    return leads;
  }, [leads, sortBy, settings.sortingRules]);

  const filtered = useMemo(() => {
    return prioritizedLeads.filter((l) => {
      if (statusFilter !== "all" && l.status !== statusFilter) return false;
      if (searchQuery) {
        const query = searchQuery.toLowerCase();
        return (
          l.patientName.toLowerCase().includes(query) ||
          getCountryName(l.patientCountry).toLowerCase().includes(query) ||
          l.treatmentSlug.toLowerCase().includes(query)
        );
      }
      return true;
    });
  }, [prioritizedLeads, statusFilter, searchQuery, t]);
                      ...
                      <div className="flex items-start justify-between gap-3">
                        <div className="min-w-0 flex-1">
                          <h3 className="flex max-w-full min-w-0 flex-row flex-nowrap items-center gap-1 text-lg font-bold leading-snug text-foreground transition-colors group-hover:text-primary">
                            <span className="min-w-0 flex-1 truncate">{lead.patientName}</span>
                          ...
                          <div className="mt-1 flex flex-wrap items-center gap-x-2 gap-y-1 text-sm">
                            <span className="flex items-center gap-1 text-muted-foreground">
                              ...
                              {getCountryName(lead.patientCountry)}
                            </span>
                            ...
                            <span className="font-medium text-foreground/80">
                              ...
                              <span className="font-normal text-muted-foreground">
                                {" "}
                                · {translatedClientType(lead.clientType, t)}
                              </span>
                            </span>
                          </div>
                        </div>
                      ...
                      <div className="min-w-0 flex-1 space-y-1">
                        ...
                        <div className="flex min-w-0 flex-wrap items-center gap-x-1.5 gap-y-0.5 text-xs text-muted-foreground">
                          <span className="flex min-w-0 max-w-full items-center gap-0.5">
                            ...
                            <span className="truncate">{getCountryName(lead.patientCountry)}</span>
                          </span>
                          <span className="text-muted-foreground/30">•</span>
