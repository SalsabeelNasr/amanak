...
export function LeadQuotationsTab({
  ...
  async function confirmApproveSend() {
    ...
    try {
      const updated = await crm.requests.sendDraftQuotationToPatient(
        lead.id,
        approveTarget.id,
        {},
      );
      ...
      console.error(e);
      setApproveError(t("requestQuotation.approveErrorGeneric"));
    } finally {
    ...
    <div className="space-y-4">
      ...
      >
        <DialogContent size="md" layout="default" showCloseButton>
          <DialogHeader>
            <DialogTitle>{t("requestQuotation.approveDialogTitle")}</DialogTitle>
            <DialogDescription>
              {t("requestQuotation.approveDialogDescription")}
            </DialogDescription>
          </DialogHeader>
          ...
          <DialogFooter className="gap-2 border-t border-border pt-4 sm:gap-0">
            ...
            >
              {t("requestQuotation.approveCancel")}
            </Button>
            <Button
              type="button"
            ...
            >
              {approveSubmitting
                ? t("requestQuotation.approveSubmitting")
                : t("requestQuotation.approveConfirm")}
            </Button>
          </DialogFooter>
        </DialogContent>
      </DialogShell>
      ...
      {displayedQuotations.map((q) => {
        ...
        const statusLabel = t(
          `requestQuotation.viewStatus.${q.status}` as Parameters<typeof t>[0],
        );
        return (
          <section
            key={q.id}
            ...
            >
              ...
              <div className="flex flex-wrap items-center gap-2">
                {q.status === "draft" && isAuthenticated ? (
                  ...
                  >
                    {t("requestQuotation.approveButton")}
                  </Button>
                ) : null}
                <Button
                  type="button"
