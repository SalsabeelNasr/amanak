...
import {
  RequestModalsProvider,
  useRequestModals,
} from "@/lib/crm/hooks/use-request-modals";
...
function LeadDetailContent({
  ...
  const [lead, setLead] = useState<Lead>(initialRequest);
    ...
    async (modalNote: string) => {
      ...
      try {
        if (pendingSkipStatus) {
          const persisted = await crm.requests.setStatus(
            lead.id,
            pendingSkipStatus,
        ...
        if (pendingTransition) {
          ...
          const persisted = await crm.requests.update(lead.id, updated, {});
          setLead(persisted);
          setPendingTransitionState(null);
          setSuccessFlash(true);
          modals.close();
  ...
  async function handleUpdateOwner(ownerId: string) {
    if (!session.isAuthenticated) return;
    try {
      const updated = await crm.requests.update(lead.id, { ownerId }, {});
      setLead(updated);
      modals.close();
    } catch (e) {
      console.error(e);
  ...
  async function handleUpdateDueDate(date: string) {
    ...
    try {
      ...
      const updated = await crm.requests.update(
        lead.id,
